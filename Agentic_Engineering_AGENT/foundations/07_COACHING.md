...
Each question has a right to a bad answer. A confident wrong answer is more useful than a correct one supplied by the coach, because only the first is diagnostic.
...
When an explanation sounds right but lands wrong, check whether two sides of one of these pairs have been treated as the same thing.
