...
| **A workflow to trust** | 🧪 [`07_VALIDATING_A_WORKFLOW.md`](07_VALIDATING_A_WORKFLOW.md) | 🔬 [verification](../foundations/Agentic_Engineering/04_VERIFICATION.md) |
...
6. **Verify, and record the evidence with its provenance.** 🔬 [verification](../foundations/Agentic_Engineering/04_VERIFICATION.md) covers the check ladder, and the rule that a required check is satisfied only by having actually run.
...
| `handbook/` | What to **do**, here, with these files | No -- specific to this package |

A rule that would hold for any agentic-engineering effort belongs in `foundations/`, in the area it fits. A procedure for getting something done with *these* files belongs here. When this file explains a rule rather than pointing at it, that is a leak and the rule should move.
