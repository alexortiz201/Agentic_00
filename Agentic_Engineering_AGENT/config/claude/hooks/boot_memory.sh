...
# ONE variable is still enough after the 2026-09-16 split. `.memory/` and `.workgroup/` moved to
# the workbench home under ~/.workbench/__SOLUTION__/; `.profile/` stayed in the workbench tool because
# it is bound for a private operator repo. This script resolves ONLY the first two -- it has never
# referenced `.profile/` -- so a second variable would name a root nothing here reads. Anything that
# does touch `.profile/` spells that path itself.
#
