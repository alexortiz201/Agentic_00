...
- **State lives in three folders across TWO roots** — `.memory/` and `.workgroup/` under `~/.workbench/__SOLUTION__/`, and `.profile/` still under `~/Projects/Agentic_00/Agentic_Engineering_AGENT/` (it is bound for a private operator repo, so it did not move) — never in the repo the session started in. Entry point: `~/.workbench/__SOLUTION__/.memory/running_context.md`.
